 ...
 */
export class System<TParams, TInternal, TData> {
    ...
    public simulate(times: number[]) {
        ...
        const result: { times: number[], values: { name: string, y: number[] }[][] } =
          { times, values: [] };
        ...
        times.forEach((t, iTime) => {
            ...
            this.iterateParticles((iParticle) => {
                    ...
                    vars.keys().forEach(v => {
                        ...
                        if (typeof val === "object" && "data" in val) {
                            for (let i = 0; i < val.data.length; i++) {
                                result.values[iParticle].push({ name: `${v}[${i}]`, y: [] });
                            }
                        } else {
                            result.values[iParticle].push({ name: v, y: [] });
                        }
                    });
                }
                ...
                vars.keys().forEach(v => {
                    ...
                    if (typeof val === "object" && "data" in val) {
                        for (let i = 0; i < val.data.length; i++) {
                            const res = result.values[iParticle].find(x => x.name === `${v}[${i}]`);
                            res?.y.push((val.data as number[])[i] as number);
                        }
                    } else {
                        const res = result.values[iParticle].find(x => x.name === v);
                        res?.y.push(vars.get(v) as number);
                    }
                })
            });
        });
